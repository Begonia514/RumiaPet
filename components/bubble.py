...
class BubbleWindow(QMainWindow):
    def __init__(self, imagePath: str, posiX: int, posiY: int):
        ...
        #
        # # Create a single-shot timer to start fading out after 3 seconds
        ...
        self.fade_out_animation.setDuration(5000)  # Fade-out time is set to 2 seconds
        self.fade_out_animation.finished.connect(self.close)
        self.fade_out_animation.start()

    # def start_display_timer(self):
    #     # Stop fade-in animation and start a single-shot timer to close after 3 seconds
    #     self.display_timer = QTimer(self)
    #     self.display_timer.singleShot(2000, self.close)
